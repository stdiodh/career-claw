...
def check_scheduled_workflow(
    ...
    *,
    ...
    require_contains(text, f"should-run-now.py --workflow {workflow_type}", path)
    require_contains(text, "CAREER_FEED_TIMEZONE", path)
    require_contains(text, "CAREER_FEED_BACKEND_DAILY_TIME", path)
    require_contains(text, "CAREER_FEED_NEWS_DAILY_TIME", path)
    ...
    if workflow_type == "backend_daily":
        require_contains(
            text,
            "CAREER_FEED_OSS_RECENT_DAYS: ${{ vars.CAREER_FEED_OSS_RECENT_DAYS }}",
            path,
        )
    ...
    require_contains(text, "actions/cache/restore@v5", path)
    require_contains(text, "actions/cache/save@v5", path)
    ...
    require_contains(text, 'now_epoch="$(TZ="${RUNTIME_TIMEZONE}" date +%s)"', path)
    require_contains(
        text,
        'target_epoch="$(TZ="${RUNTIME_TIMEZONE}" date -d "${RUNTIME_LOCAL_DATE} ${TARGET_TIME}:00" +%s)"',
        path,
    )
    require_contains(text, 'sleep "${wait_seconds}"', path)
    require_contains(text, "sent_at_local=", path)
    require_contains(text, "sent_at_kst=", path)
    ...
    print(f"ok: {label} runtime schedule = {cron} with {workflow_type} gate")
...
def check_weekly_site_radar_workflow() -> None:
    ...
    if not has_quoted_value(text, "cron", "5,35 * * * *"):
        fail(f"{WEEKLY_WORKFLOW}: missing runtime wake-up schedule")
    ...
    require_contains(text, "Check runtime schedule", WEEKLY_WORKFLOW)
    require_contains(text, "should-run-now.py --workflow career_weekly", WEEKLY_WORKFLOW)
    ...
    require_contains(text, "discord_delivery_disabled", WEEKLY_WORKFLOW)
    ...
    print("ok: Backend Career Site Radar has runtime schedule and workflow_dispatch")
