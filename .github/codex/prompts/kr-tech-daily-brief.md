...
- `spring-study-topic.json`의 `today.track`, `today.level`, `today.one_line_question`, `today.problem_situation`, `today.official_doc_keywords`, `today.learning_steps_30m`, `today.practice_steps_30m`, `today.blog_title_candidates`, `today.paar_outline`, `today.done_criteria`, `today.next_topic`을 우선 사용한다.
- `diagnostics.fallback_used=true`이면 `fallback_reasons`를 확인하고, 후보가 얕은 이유를 보완해 더 작은 블로그 주제로 좁힌다.
...
- `30분 학습`은 문서 읽기 목표를 최소 2개 이상의 행동으로 쪼갠다.
- `30분 실습`은 작은 코드, 설정, 로그, 테스트, curl, DB 쿼리처럼 손으로 확인 가능한 행동을 최소 2개 이상 제시한다.
