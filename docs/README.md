# Documentation

Career Feed 문서는 사용자 목적별로 나뉩니다.

처음 온 사용자는 getting started 문서부터 읽고, 운영 정책이나 기여 기준은 필요한 시점에 따라가면 됩니다.

## Start here

| Document | Purpose |
| --- | --- |
| [Fork Setup Guide](./getting-started/fork-setup.md) | fork 후 첫 dry-run과 Discord delivery 설정 |
| [Usage Guide](./getting-started/usage.md) | workflow 실행, artifact 확인, dry-run 사용법 |
| [Runtime Configuration](./getting-started/runtime-configuration.md) | Secrets, Variables, runtime gate 설정 |
| [Sample Output](./getting-started/sample-output.md) | 생성 결과 예시와 검토 포인트 |
| [Demo Guide](./demo.md) | redacted demo screenshot과 공개 데모 기준 |
| [Demo Assets](./assets/demo/README.md) | demo screenshot/GIF asset 관리 기준 |

## Operations

| Document | Purpose |
| --- | --- |
| [Daily Backend Brief](./operations/daily-backend-brief.md) | 일일 백엔드 브리프 운영 |
| [Korea Dev/AI News Daily](./operations/daily-news-ops.md) | 뉴스 브리프 운영 |
| [Backend Career Site Radar](./operations/career-site-radar.md) | 커리어 레이더 운영 |
| [Daily Growth Ops](./operations/daily-growth-ops.md) | Daily growth section 운영 확인 |
| [Operations Guide](./operations/operations.md) | 운영 흐름과 체크포인트 |
| [Local Validation](./operations/local-validation.md) | 로컬 검증 명령 |
| [Maintainer Guide](./operations/maintainer-guide.md) | maintainer 운영 체크리스트 |

## Policies

| Document | Purpose |
| --- | --- |
| [OSS Candidate Policy](./policies/oss-candidate-policy.md) | OSS 후보 안전 정책 |
| [Spring/JVM Blog Topic Policy](./policies/daily-spring-jvm-blog-topic-policy.md) | Daily Spring/JVM 주제 정책 |
| [GitHub Labels](./policies/github-labels.md) | label 운영 기준 |
| [Validation Policy](./operations/daily-backend-brief.md) | generated brief와 safe OSS 후보 검증 정책 |
| [Security Policy](../SECURITY.md) | secret과 webhook 취급 |
| [Support](../SUPPORT.md) | 질문, 버그 리포트, 지원 범위 |

## Contributing

| Document | Purpose |
| --- | --- |
| [CONTRIBUTING.md](../CONTRIBUTING.md) | 전체 기여 방식과 PR 기준 |
| [Contributor Task Ideas](./project/contributor-tasks.md) | 작은 기여 후보 |
| [Contribution Guide Index](./contributing/README.md) | 기여 세부 문서 index |
| [Good Suggestion Criteria](./contributing/good-suggestion-criteria.md) | 좋은 제안의 기준 |
| [Source Suggestion Guide](./contributing/source-suggestion-guide.md) | 출처 제안 기준 |
| [OSS Candidate Guide](./contributing/oss-candidate-guide.md) | OSS 후보 제안 기준 |
| [Backend Career Question Guide](./contributing/backend-career-question-guide.md) | 백엔드 커리어 질문 작성 기준 |
| [Review Policy](./contributing/review-policy.md) | maintainer review 기준 |

## Project

| Document | Purpose |
| --- | --- |
| [Roadmap](./project/roadmap.md) | 향후 방향 |
| [Changelog](../CHANGELOG.md) | 변경 이력 |
| [Release Checklist](./project/release-checklist.md) | maintainer용 릴리스 전 점검 |
| [v0.1.0 Release Notes](./release-notes/v0.1.0.md) | release note 초안 |
| [Ecosystem Importance](./project/ecosystem-importance.md) | 백엔드 생태계에서의 의미와 한계 |
| [Open Source Readiness Review](./project/open-source-readiness-review.md) | 공개 준비 상태 검토 |

## Archive / Legacy

레거시 문서는 다음 단계에서 별도로 정리합니다.

이번 단계에서는 삭제하지 않습니다.

| Document | Purpose |
| --- | --- |
| [Legacy Inventory](./legacy-inventory.md) | 레거시 문서와 제거 후보 기록 |
| [OSS Program Application Notes](./oss-program-application.md) | 지원 프로그램 신청용 참고 문서 |
| [Community Guide](./community-guide.md) | 스터디와 커뮤니티 재사용 참고 |
| [Backend Growth Curriculum](./backend-growth-curriculum.md) | 백엔드 성장 커리큘럼 참고 |

## Repository level documents

| Document | Purpose |
| --- | --- |
| [README](../README.md) | 프로젝트 랜딩과 Quick Start |
| [Security](../SECURITY.md) | secret, webhook, API key 취급 |
| [Support](../SUPPORT.md) | 지원 범위와 문의 방식 |
| [Code of Conduct](../CODE_OF_CONDUCT.md) | 커뮤니티 행동 기준 |
| [License](../LICENSE) | 라이선스 |
