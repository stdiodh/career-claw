...
- `spring-study-topic.json`은 `spring-jvm-study-topics` 카테고리에서 생성하며 Naver query를 사용하지
  않습니다.
- Spring 공식 블로그, Spring 문서, OpenJDK/Inside Java, Micrometer/OpenTelemetry 등 공식·표준 레퍼런스를
  우선합니다.
...
- `backend-practical-knowledge.json`, `cs-core-daily-topic.json`,
  `backend-term-daily.json`은 계속 생성합니다.
...
- 이후 Gradle, Ktor Documentation, Quarkus, Testcontainers Java, Micronaut Core, Spring
  Framework 순서로 확장합니다.
