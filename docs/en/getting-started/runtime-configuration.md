...
Career Feed forks can run the first Backend Daily dry-run with only `OPENAI_API_KEY`, then configure schedule times and Discord delivery without editing workflow YAML.
...
| Type | Store | Examples |
| --- | --- | --- |
| Secrets | API keys, webhook URLs, client secrets | `OPENAI_API_KEY`, `DISCORD_WEBHOOK_CAREER_FEED` |
| Variables | Locale list, provider names, timezone, target time, delivery flag | `CAREER_FEED_ENABLED_LOCALES`, `CAREER_FEED_SCHEDULE_ENABLED` |

Do not put Discord Webhook URLs, API keys, or client secrets in Variables.

## Secrets

For the first manual Backend Daily dry-run, only `OPENAI_API_KEY` is required.

Discord webhook, Naver, and Brave Secrets are optional and can be added after artifact review.

| Secret | Required | Used by |
| --- | --- | --- |
| `OPENAI_API_KEY` | Required for first dry-run | Daily Backend Brief and News Daily generation |
...
| `CAREER_FEED_SCHEDULE_ENABLED` | Optional | `false` | `true` | Enables recurring scheduled generation |
...
Do not create Variables for the first smoke test unless you intentionally need an override.
...
Scheduled generation is disabled by default. Scheduled events return `should_run=false` with `reason=schedule_disabled` unless `CAREER_FEED_SCHEDULE_ENABLED=true`.
...
Manual `workflow_dispatch` runs are not blocked by `CAREER_FEED_SCHEDULE_ENABLED`, the runtime time window, or weekday checks.

Invalid configuration values produce safe skip behavior before expensive generation starts.
...
| `schedule_enabled` | Whether recurring scheduled generation is enabled |
...
4. Missing Discord webhook Secret blocks delivery with a clear error.
5. Delivery can proceed only after validation passes.

The default delivery flag is `false` to prevent accidental fork sends.

Webhook precedence is specific locale/feed Secret, legacy fallback Secret, then `DISCORD_WEBHOOK_CAREER_FEED`.
...
`CAREER_FEED_DISCORD_DELIVERY_ENABLED=true` does not override `dry_run=true`.
...
- Waiting for a scheduled run while `CAREER_FEED_SCHEDULE_ENABLED=false`.
...
Runtime gate, schedule flag, and delivery flag control whether workflows continue and whether Discord delivery is attempted.
