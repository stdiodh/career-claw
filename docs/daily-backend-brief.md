...
- `backend-practical-knowledge.json`, `cs-core-daily-topic.json`, `backend-term-daily.json`은 계속
  생성합니다.
...
- 이후 Gradle, Ktor Documentation, Quarkus, Testcontainers Java, Micronaut Core, Spring Framework 순서로
  확장합니다.
...
저장소 profile, scoring, diagnostics, safe candidate gate는 [OSS 후보 저장소 정책](./oss-candidate-policy.md)을
따릅니다.
...
- Markdown의 OSS issue URL이 `kr-oss-contribution-opportunities.json`의 `safe_to_recommend=true` 후보
  URL과 다르면 실패합니다.
